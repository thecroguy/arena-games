...
export default function Home() {
  ...
  useEffect(() => {
    if (sideTab === 'chat') chatEndRef.current?.scrollIntoView({ behavior: 'smooth' })
  }, [chat, sideTab])
  ...
  return (
    <div style={{ background:'#08080f', minHeight:'100vh', color:'#e2e8f0' }}>
      ...
      {/* ── HUD bar ─────────────────────────────────────────────────── */}
      <div style={{ background:'#0a0a14', borderBottom:'1px solid #0f0f1e', display:'flex', alignItems:'center', height:'40px', padding:'0 clamp(10px,2.5vw,24px)', justifyContent:'space-between' }}>
        <div style={{ display:'flex' }}>
          {[
            ...
            <div key={i} style={{ display:'flex', alignItems:'center', gap:'6px', padding:'0 clamp(8px,1.8vw,18px)', borderRight:i<3?'1px solid #0f0f1e':'none' }}>
              ...
              <span style={{ fontSize:'0.52rem', color:'#242438', fontFamily:'Orbitron,sans-serif', letterSpacing:'0.1em' }}>{s.l}</span>
              <span style={{ fontSize:'0.8rem', fontWeight:900, color:s.c, fontFamily:'Orbitron,sans-serif' }}>{s.v}</span>
            </div>
          ))}
        </div>
        <span style={{ fontSize:'0.55rem', color:'#1e1e30', fontFamily:'Orbitron,sans-serif', letterSpacing:'0.08em' }}>USDT · POLYGON · BSC · ETH · MORE</span>
      </div>

      {/* ── Main ────────────────────────────────────────────────────── */}
      <div style={{ padding:'clamp(12px,2vw,20px) clamp(10px,2.5vw,24px)', display:'grid', gridTemplateColumns:'minmax(0,1fr)', gap:'14px' }}>
            ...
            return (
              <button key={g.id} className="game-tab"
                ...
                style={{
                  ...
                  fontFamily:'Orbitron,sans-serif', fontSize:'0.62rem', fontWeight:700, letterSpacing:'0.06em',
                  boxShadow: active ? `0 4px 18px rgba(${g.glowRgb},0.22)` : 'none',
                }}>
                <span style={{ fontSize:'0.95rem' }}>{g.emoji}</span>
                {g.short}
                {g.hot && <span style={{ fontSize:'0.48rem', padding:'2px 5px', borderRadius:'4px', background:'rgba(239,68,68,0.18)', color:'#ef4444', border:'1px solid rgba(239,68,68,0.3)', animation:'hot-badge 1.6s ease-in-out infinite' }}>HOT</span>}
              </button>
            )
          })}
        </div>

        {/* 2-col: featured + sidebar */}
        <div style={{ display:'grid', gridTemplateColumns:'minmax(0,1fr) clamp(230px,22vw,310px)', gap:'12px' }} className="main-grid">
          <style>{`@media(max-width:800px){.main-grid{grid-template-columns:1fr!important}.right-col{display:none!important}}`}</style>

          {/* ── Featured card (the alive one) ─────────────── */}
          <div key={activeGame.id} style={{ position:'relative', borderRadius:'20px', overflow:'hidden', border:`1px solid rgba(${activeGame.glowRgb},0.22)`, background:'#0c0c17', animation:'slide-in .22s ease-out', minHeight:'clamp(300px,38vh,460px)', display:'flex', flexDirection:'column' }}>
            ...
            <div style={{ position:'relative', zIndex:2, padding:'clamp(18px,2.5vw,32px)', flex:1, display:'flex', flexDirection:'column', justifyContent:'space-between' }}>
              <div>
                <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', marginBottom:'18px', flexWrap:'wrap', gap:'8px' }}>
                  <div style={{ display:'flex', alignItems:'center', gap:'14px' }}>
                    <div style={{ width:'64px', height:'64px', borderRadius:'16px', flexShrink:0, background:`linear-gradient(135deg,rgba(${activeGame.glowRgb},0.25),rgba(${activeGame.glowRgb},0.07))`, border:`1px solid rgba(${activeGame.glowRgb},0.32)`, display:'flex', alignItems:'center', justifyContent:'center', fontSize:'2.1rem', boxShadow:`0 0 22px rgba(${activeGame.glowRgb},0.25)`, animation:'float-emoji 3s ease-in-out infinite' }}>
                      {activeGame.emoji}
                    </div>
                    <div>
                      <h1 style={{ fontFamily:'Orbitron,sans-serif', fontWeight:900, fontSize:'clamp(1.2rem,2.8vw,1.9rem)', margin:0, lineHeight:1.1, background:`linear-gradient(135deg,#fff 30%,${activeGame.glow})`, WebkitBackgroundClip:'text', WebkitTextFillColor:'transparent', backgroundClip:'text' }}>
                        {activeGame.title}
                      </h1>
                      <div style={{ display:'flex', gap:'5px', marginTop:'6px', flexWrap:'wrap' }}>
                        {activeGame.tags.map(t => (
                          <span key={t} style={{ fontSize:'0.6rem', fontWeight:700, padding:'2px 9px', borderRadius:'20px', background:`rgba(${activeGame.glowRgb},0.12)`, color:activeGame.glow, border:`1px solid rgba(${activeGame.glowRgb},0.22)`, letterSpacing:'0.05em' }}>{t}</span>
                        ))}
                        {activeGame.hot && <span style={{ fontSize:'0.6rem', fontWeight:700, padding:'2px 9px', borderRadius:'20px', background:'rgba(239,68,68,0.13)', color:'#ef4444', border:'1px solid rgba(239,68,68,0.28)', animation:'hot-badge 1.6s ease-in-out infinite' }}>🔥 HOT</span>}
                      </div>
                    </div>
                  </div>
                  <div style={{ display:'flex', alignItems:'center', gap:'5px', padding:'5px 11px', borderRadius:'8px', background:'rgba(34,197,94,0.07)', border:'1px solid rgba(34,197,94,0.16)' }}>
                    ...
                    <span style={{ fontSize:'0.6rem', fontFamily:'Orbitron,sans-serif', fontWeight:700, color:'#22c55e', letterSpacing:'0.08em' }}>LIVE</span>
                  </div>
                </div>

                <p style={{ color:'#64748b', fontSize:'0.88rem', lineHeight:1.7, marginBottom:'20px', maxWidth:'500px' }}>{activeGame.desc}</p>
                ...
                <div style={{ display:'flex', gap:'8px', flexWrap:'wrap', marginBottom:'24px' }}>
                  {[
                    ...
                    <div key={i} style={{ padding:'9px 14px', borderRadius:'11px', background: s.hi ? `rgba(${activeGame.glowRgb},0.1)` : 'rgba(255,255,255,0.03)', border: s.hi ? `1px solid rgba(${activeGame.glowRgb},0.22)` : '1px solid #0f0f1e', display:'flex', flexDirection:'column', gap:'2px', minWidth:'82px' }}>
                      <span style={{ fontSize:'0.52rem', color:'#242438', fontFamily:'Orbitron,sans-serif', letterSpacing:'0.1em' }}>{s.label}</span>
                      <span style={{ fontSize:'0.85rem', fontWeight:800, color: s.hi ? activeGame.glow : '#e2e8f0', fontFamily:'Orbitron,sans-serif', display:'flex', alignItems:'center', gap:'4px' }}>
              ...
              <div style={{ display:'flex', gap:'9px', flexWrap:'wrap', alignItems:'center' }}>
                <button className="play-btn" onClick={() => navigate(`/lobby/${activeGame.id}`)}
                  style={{ background:`linear-gradient(135deg,${activeGame.bgFrom},${activeGame.bgTo})`, border:'none', borderRadius:'11px', padding:'13px 34px', color:'#fff', fontFamily:'Orbitron,sans-serif', fontWeight:900, fontSize:'0.88rem', cursor:'pointer', letterSpacing:'0.07em', boxShadow:`0 0 30px rgba(${activeGame.glowRgb},0.45),0 4px 14px rgba(0,0,0,0.4)` }}>
                  PLAY NOW →
                </button>
                <button className="bot-btn play-btn" onClick={() => navigate('/game/practice', { state:{ bot:true, entry:0, gameMode:activeGame.id } })}
                  style={{ background:'rgba(124,58,237,0.07)', border:'1px solid rgba(124,58,237,0.18)', borderRadius:'11px', padding:'13px 20px', color:'#a78bfa', fontWeight:700, fontSize:'0.8rem', cursor:'pointer', fontFamily:'Orbitron,sans-serif' }}>
                  vs Bot
                </button>
                <span style={{ fontSize:'0.65rem', color:'#1e1e30' }}>No wallet needed</span>
              </div>
            </div>
            <div style={{ position:'absolute', right:'-14px', bottom:'-8px', fontSize:'150px', opacity:0.04, pointerEvents:'none', userSelect:'none', lineHeight:1 }}>{activeGame.emoji}</div>
          </div>

          {/* ── Right column: game list + chat ─────────────── */}
          <div className="right-col" style={{ display:'flex', flexDirection:'column', gap:'0', background:'#09090f', border:'1px solid #0f0f1e', borderRadius:'18px', overflow:'hidden' }}>

            {/* Tab header */}
            <div style={{ display:'flex', alignItems:'center', padding:'10px 12px', gap:'4px', borderBottom:'1px solid #0f0f1e', flexShrink:0 }}>
              {(['games','chat','activity'] as const).map(t => (
                <button key={t} className="tab-btn"
                  onClick={() => setSideTab(t)}
                  style={{ padding:'5px 10px', borderRadius:'7px', fontSize:'0.58rem', fontFamily:'Orbitron,sans-serif', fontWeight:700, letterSpacing:'0.05em', background: sideTab===t ? 'rgba(124,58,237,0.2)' : 'transparent', color: sideTab===t ? '#a78bfa' : '#2d2d42', border: sideTab===t ? '1px solid rgba(124,58,237,0.28)' : '1px solid transparent' }}>
              ...
              {sideTab !== 'games' && (
                <div style={{ marginLeft:'auto', display:'flex', alignItems:'center', gap:'4px' }}>
                  ...
                  <span style={{ fontSize:'0.58rem', color:'#22c55e', fontFamily:'Orbitron,sans-serif', fontWeight:700 }}>{onlineCount||'—'}</span>
                </div>
              )}
            </div>
              ...
              <div style={{ flex:1, overflowY:'auto', padding:'8px 10px', display:'flex', flexDirection:'column', gap:'5px' }}>
                {GAMES.filter(g => g.id !== activeGame.id).map(g => (
                  <button key={g.id} className="side-row"
                    onClick={() => setActiveGame(g)}
                    style={{ display:'flex', alignItems:'center', gap:'10px', padding:'10px 12px', background:'rgba(255,255,255,0.02)', border:`1px solid rgba(${g.glowRgb},0.1)`, borderRadius:'11px', cursor:'pointer', textAlign:'left', width:'100%' }}>
                    <div style={{ width:'34px', height:'34px', borderRadius:'9px', background:`rgba(${g.glowRgb},0.12)`, border:`1px solid rgba(${g.glowRgb},0.18)`, display:'flex', alignItems:'center', justifyContent:'center', fontSize:'1.05rem', flexShrink:0 }}>{g.emoji}</div>
                    <div style={{ minWidth:0, flex:1 }}>
                      <div style={{ fontFamily:'Orbitron,sans-serif', fontSize:'0.65rem', fontWeight:700, color:'#94a3b8', letterSpacing:'0.03em', whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>{g.title}</div>
                      <div style={{ display:'flex', alignItems:'center', gap:'4px', marginTop:'2px' }}>
                        ...
                        <span style={{ fontSize:'0.58rem', color:'#2d2d42' }}>{g.activePlayers} active</span>
                      </div>
                    </div>
                    {g.hot && <span style={{ fontSize:'0.48rem', padding:'2px 5px', borderRadius:'4px', background:'rgba(239,68,68,0.14)', color:'#ef4444', flexShrink:0, border:'1px solid rgba(239,68,68,0.22)' }}>HOT</span>}
                    <span style={{ color:`rgba(${g.glowRgb},0.55)`, fontSize:'0.8rem', flexShrink:0 }}>›</span>
                  </button>
                ))}
                {/* platform stats */}
                <div style={{ marginTop:'6px', padding:'12px', background:'rgba(124,58,237,0.04)', border:'1px solid rgba(124,58,237,0.1)', borderRadius:'10px' }}>
                  <div style={{ fontFamily:'Orbitron,sans-serif', fontSize:'0.5rem', color:'#1e1e30', letterSpacing:'0.12em', marginBottom:'8px' }}>PLATFORM</div>
                  {[{ l:'Total wagered', v:'$284,710', c:'#a78bfa' }, { l:'Games played', v:'18,432', c:'#06b6d4' }, { l:'Biggest pot', v:'$1,275', c:'#f59e0b' }].map(s => (
                    <div key={s.l} style={{ display:'flex', justifyContent:'space-between', padding:'4px 0', borderBottom:'1px solid #0c0c14' }}>
                      <span style={{ fontSize:'0.62rem', color:'#2d2d42' }}>{s.l}</span>
                      <span style={{ fontSize:'0.65rem', fontWeight:700, color:s.c, fontFamily:'Orbitron,sans-serif' }}>{s.v}</span>
                    </div>
                  ))}
                  <Link to="/leaderboard" style={{ display:'block', textAlign:'center', marginTop:'8px', fontSize:'0.62rem', color:'#2d2d42', textDecoration:'none', fontWeight:600 }}>🏆 Leaderboard →</Link>
                </div>
              </div>
            )}
            ...
            {sideTab === 'chat' && (
              <>
                <div style={{ flex:1, overflowY:'auto', padding:'10px 12px', display:'flex', flexDirection:'column', gap:'7px' }}>
                  {chat.length === 0
                    ? <div style={{ textAlign:'center', color:'#1e1e30', fontSize:'0.72rem', marginTop:'30px' }}>Connecting…</div>
                    : chat.map((m, i) => {
            ...
            {sideTab === 'activity' && (
              <div style={{ flex:1, overflowY:'auto', padding:'8px 12px', display:'flex', flexDirection:'column', gap:'0' }}>
                {activity.length === 0
                  ? <div style={{ textAlign:'center', color:'#1e1e30', fontSize:'0.72rem', marginTop:'30px' }}>No activity yet…</div>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(120px,1fr))', gap:'7px' }} className="mob-grid">
          <style>{`@media(min-width:801px){.mob-grid{display:none!important}}`}</style>
          {GAMES.filter(g => g.id !== activeGame.id).map(g => (
            <button key={g.id} onClick={() => setActiveGame(g)}
              style={{ background:`rgba(${g.glowRgb},0.07)`, border:`1px solid rgba(${g.glowRgb},0.16)`, borderRadius:'13px', padding:'13px 8px', cursor:'pointer', display:'flex', flexDirection:'column', alignItems:'center', gap:'6px' }}>
              <span style={{ fontSize:'1.5rem' }}>{g.emoji}</span>
              <span style={{ fontFamily:'Orbitron,sans-serif', fontSize:'0.55rem', fontWeight:700, color:g.glow, letterSpacing:'0.04em' }}>{g.short}</span>
            </button>
          ))}
        </div>
        ...
        <div>
          <div style={{ display:'flex', alignItems:'center', gap:'8px', marginBottom:'10px' }}>
            <span style={{ fontFamily:'Orbitron,sans-serif', fontWeight:900, fontSize:'0.68rem', color:'#e2e8f0', letterSpacing:'0.1em' }}>ARENA INTEL</span>
            <span style={{ fontSize:'0.55rem', padding:'2px 8px', background:'rgba(124,58,237,0.1)', color:'#a78bfa', borderRadius:'20px', border:'1px solid rgba(124,58,237,0.2)', fontWeight:700 }}>ARTICLES</span>
            <span style={{ fontSize:'0.55rem', color:'#1e1e30', marginLeft:'2px' }}>Coming soon</span>
          </div>
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(220px,1fr))', gap:'9px' }}>
            {ARTICLES.map((a, i) => (
              <div key={i} className="art-card"
                style={{ background:'#0c0c17', border:'1px solid #111120', borderRadius:'13px', padding:'15px', position:'relative', overflow:'hidden' }}>
                ...
                <span style={{ fontSize:'0.52rem', padding:'2px 8px', borderRadius:'20px', background:`rgba(${a.cRgb},0.1)`, color:a.color, border:`1px solid rgba(${a.cRgb},0.22)`, fontWeight:700, fontFamily:'Orbitron,sans-serif', letterSpacing:'0.06em' }}>{a.tag}</span>
                <div style={{ marginTop:'9px', fontSize:'0.78rem', color:'#94a3b8', lineHeight:1.5, fontWeight:600, marginBottom:'7px' }}>{a.title}</div>
                <div style={{ fontSize:'0.6rem', color:'#1e1e30' }}>📖 {a.time} read</div>
              </div>
            ))}
          </div>
        </div>
        ...
        {/* ── How it works ─────────────────────────────────────────── */}
        <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:'9px', paddingTop:'6px', borderTop:'1px solid #0a0a14' }} className="how-grid">
          <style>{`@media(max-width:520px){.how-grid{grid-template-columns:1fr!important}}`}</style>
          {[
            { n:'01', icon:'💳', title:'CONNECT', desc:'Any EVM wallet — MetaMask, Coinbase, WalletConnect.' },
            { n:'02', icon:'🎮', title:'COMPETE', desc:'Pick a game, pay USDT, play against real opponents.' },
            { n:'03', icon:'🏆', title:'CLAIM',   desc:'Winner takes 85% of the pot. On-chain. Instant.' },
          ].map(s => (
            <div key={s.n} style={{ background:'rgba(255,255,255,0.015)', border:'1px solid #0c0c14', borderRadius:'13px', padding:'16px', position:'relative', overflow:'hidden' }}>
              <div style={{ position:'absolute', top:'9px', right:'11px', fontFamily:'Orbitron,sans-serif', fontSize:'0.5rem', color:'#141420', letterSpacing:'0.15em' }}>{s.n}</div>
              <div style={{ fontSize:'1.4rem', marginBottom:'7px' }}>{s.icon}</div>
              <div style={{ fontFamily:'Orbitron,sans-serif', fontSize:'0.67rem', fontWeight:700, color:'#e2e8f0', marginBottom:'4px', letterSpacing:'0.06em' }}>{s.title}</div>
              <div style={{ color:'#475569', fontSize:'0.75rem', lineHeight:1.55 }}>{s.desc}</div>
            </div>
          ))}
        </div>
      </div>
